...
datt <-read.csv("C:/Hawaii_project/additional_studies/HawIslandsAbundance_2SizeClasses_100plus_v2_1.csv" ) 
datt$PlotID <- as.factor(datt$PlotID) 
datt$Clust_manu <- datt$Clust_manu %>% replace_na('anone')

length(unique(datt$PlotID)) # 552 
...
names <- unique(datt$Scientific_name) 
...
# write.csv(datt1, file = "C:/Hawaii_project/additional_studies/HawIslandsAbundance_2SizeClasses_100plus_v2_2.csv" ,  row.names = FALSE) 
...
datt <- filter(datt, Study != "Hatheway_1952") 
datt <- filter(datt, Family != "Cibotiaceae" & Family != "Blechnaceae" & Family != "Cyatheaceae") # 1860
datt <- filter(datt, Scientific_name != "unknown_ilexlike") 
datt <- filter(datt, geo_entity2 != "Kaua'i Island")
datt <- filter(datt, Scientific_name != "Melicope kaalaensis" & Scientific_name != "Psychotria kaduana") 
...
hawaii.tree_native <- phylo.maker(spp_nat, scenarios="S2") # 79 native species. 

hawaii.tree_40    <- phylo.maker(spp_nat, scenarios="S2") # 118 native species. 

hawaii.tree_all   <- phylo.maker(spp_all, scenarios="S2") # 146 native species. 
